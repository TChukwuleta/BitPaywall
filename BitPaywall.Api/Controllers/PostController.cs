...
namespace BitPaywall.Api.Controllers
{
    ...
    public class PostController : ApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<Result>> GetAllPublishedPost(int skip, int take, string userid)
        {
            try
            {
                accessToken.ValidateToken(userid);
                return await _mediator.Send(new GetAllUserPublishedPostQuery { Skip = skip, Take = take, UserId = userid, AccessToken = accessToken.RawData });
            }
            catch (Exception ex)
            {
                return Result.Failure($"Published posts retrieval failed. Error: {ex?.Message ?? ex?.InnerException?.Message}");
            }
        }
    }
}